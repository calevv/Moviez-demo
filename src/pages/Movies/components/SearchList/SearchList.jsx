...
const SearchList = ({ keyword }) => {
    ...
    const { selectedGenre, setSelectedGenre, setSelectedPopular, selectedPopular } = useFilterStore();
    ...
    const filteredMovies = useMemo(() => {
        ...
        return selectedGenre ? data.results.filter((item) => item.genre_ids?.includes(selectedGenre.id)) : data.results;
    ...
    const pageCount = selectedGenre
        ? Math.ceil((sortedMovies?.length || 0) / (data?.results?.length > 0 ? data.results.length : 20))
        : data?.total_pages || 0;
    return (
        <div>
            {isLoading ? (
